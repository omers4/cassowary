...
@click.option('-p', '--port', default=8000, type=int, help='Port to connect API server with')
...
@cli.command()
@click.option('-h', '--host', default='127.0.0.1', help='API Server hostname / IP')
@click.option('-p', '--port', default=8000, type=int, help='Port to connect run API server with')
...
def get_result(host: str, port: int, save: str, user_id: int, snapshot_id: int, parser_name: str):
    ...
    :param port: Port to connect API server with
    """
    response = requests.get(f'http://{host}:{port}/users/{user_id}/snapshots/{snapshot_id}/{parser_name}')
    if response.status_code == requests.codes.ok:
