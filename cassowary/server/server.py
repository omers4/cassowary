...
def dump_snapshot(user, snapshot):
    ...
    raw_color_path = utils.get_path(dir_path, user.user_id,
                                    snapshot.timestamp, 'raw_color_image')
    raw_depth_path = utils.get_path(dir_path, user.user_id,
                                    snapshot.timestamp, 'raw_depth_image')
    color_path = utils.get_path(dir_path, user.user_id,
                                snapshot.timestamp, 'color_image.jpg')
    depth_path = utils.get_path(dir_path, user.user_id,
                                snapshot.timestamp, 'depth_image.jpg')
...
class Handler(threading.Thread):
    ...
    def run(self):
        with self.connection as connection:
            ...
            snapshot = Snapshot.deserialize(connection.receive(),
                                            config.fields)
