...
docker run -d -p 0.0.0.0:8000:8000/tcp -v ParsedResultsVolume:/parsed_results --net cassowarrynet cassowary:server
docker run -d -e PARSER=pose --net cassowarrynet cassowary:parsers
docker run -d -e PARSER=color_image -v ParsedResultsVolume:/parsed_results --net cassowarrynet cassowary:parsers
docker run -d -e PARSER=depth_image -v ParsedResultsVolume:/parsed_results --net cassowarrynet cassowary:parsers
...
docker run -d -p 8888:8888 -v ParsedResultsVolume:/parsed_results --net cassowarrynet cassowary:api
docker run -d -p 8080:8080 -v ParsedResultsVolume:/parsed_results --net cassowarrynet cassowary:gui
